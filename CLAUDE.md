...
- **Endpoints**: `POST /api/generate-code/stream` (SSE: progress / agent_activity / validation / done / error), `GET /api/generate-code/status` (drives the frontend button's disabled state), `DELETE /api/generate-code/cache`. Router: `backend/app/routers/codegen.py`.
